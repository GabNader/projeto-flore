...
class AppCartComponent extends HTMLElement {
    // MUDANÇA AQUI: Métodos auxiliares _loadCartFromLocalStorage e _saveCartToLocalStorage
    // movidos para o início da definição da classe
    ...
    constructor() {
        ...
        this.cartItems = this._loadCartFromLocalStorage(); // Agora esse método já foi definido!
    ...
    openCart() {
        ...
        if (!this.overlay || !this.sidebar) {
            console.error('AppCartComponent: ERRO! Elementos overlay ou sidebar não encontrados para abertura. Renderizando novamente.');
            // Tenta renderizar novamente o esqueleto HTML se os elementos não foram encontrados
            this._initialRender(); 
            this.connectedCallback(); // Re-executa connectedCallback para re-selecionar elementos e re-adicionar listeners
            return;
        }
    ...
    increaseQuantity(itemId) {
        ...
        if (item) {
            item.quantity++;
            this.updateCartDisplay();
            this._saveCartToLocalStorage(); // Salva no localStorage após alteração
    ...
    addItem(item) {
        ...
        this.updateCartDisplay();
        this._saveCartToLocalStorage(); // Salva no localStorage após adição
    }
